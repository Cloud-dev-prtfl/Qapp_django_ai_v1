...
def clean_json_string(json_string):
    ...
    - Escapes unescaped control characters (newlines, tabs) inside strings.
    """
    if not json_string:
        return ""
    ...
    # 3. Attempt to fix unescaped newlines inside strings.
    # This regex looks for newlines that are NOT part of the JSON structure.
    # It's a heuristic: simple cleaning to escape control characters 0x00-0x1f
    # We rely on strict=False in json.loads usually, but let's try to escape raw newlines 
    # if they occur inside quotes.
    
    # Simple replace for common issues if the model returns raw newlines in text
    # (Note: A robust solution requires a parser, but this helps simple cases)
    # For now, we rely heavily on the prompt and 'response_format' enforcement.
    
    return json_string

def generate_questions_agent(exam_session, feedback_history=None):
    """
    Agent 1: Generates questions.
    Uses 'gemini-2.0-flash-exp' for high speed and cutting-edge capabilities.
    """
    ...
    **REQUIRED JSON STRUCTURE:**
    ...
    try:
        ...
        # (though typically not recommended for pure JSON, it helps with LLM output)
        return json.loads(cleaned_content, strict=False)

    except json.JSONDecodeError as je:
        print(f"Agent 1 JSON Decode Error: {je}")
        print(f"Faulty Content Snippet: {raw_content[:500]}...") # Log start of content for debug
        return None
...
def evaluate_agent(questions_data, exam_session):
    ...
    Uses 'gemini-1.5-pro' for superior reasoning on quality checks.
    """
...
def format_html_agent(questions_data):
    ...
    Uses 'gemini-1.5-pro' to ensure complex HTML structures are generated correctly.
    """
    ...
    **Styling Classes (Mandatory):**
    ...
    try:
        ...
        # Clean markdown if present (Formatter might not use JSON mode, so we clean manually)
...
def orchestrated_exam_flow(exam_session):
    """
    Coordinator Function:
    Manages the loop: Generate -> Evaluate -> (Retry if needed) -> Format.
    Enforces strict 20-second timeout.
    """
    ...
    while (time.time() - start_time) < max_loop_duration:
        ...
        # 2. Evaluate (Agent 2a)
        score, current_feedback = evaluate_agent(draft, exam_session)
    ...
    if not best_draft:
        return "<p class='error'>Failed to generate a valid exam within the time limit.</p>"

    print(f"⏱️ Generation Loop Ended. Finalizing with Best Score: {best_score}")
    
    # 5. Format the best draft (Agent 2b)
